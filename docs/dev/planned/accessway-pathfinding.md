...
`G` is tile-based while V1 generated designations are origin-based, so the transition between them is a seam check rather than another origin-to-origin edge. A V node may expose adjacent G nodes once enough of the V footprint has reached the current ground surface:

* When starting **below** ground and mining upward/outward, a G handoff is available when the V center point, or at least two relevant V corners, are **at or above** the adjacent ground height.
* When starting **above** ground and dumping downward/outward, a G handoff is available when the V center point, or at least two relevant V corners, are **at or below** the adjacent ground height.

This captures the point where the generated accessway has physically met vanilla pathable ground. The same rule is symmetric for `G -> V`: the G tile provides the fixed ground-side height, and the candidate V node must expose a compatible center point or at least two compatible corners on the shared side.
...
For V1, expansion implements this as a local feasibility check after converting `(origin, h, mode)` to its four candidate corner heights: each corner is tested against one deduplicated index of active designation corners and fixed building-foundation samples, and any hit rejects the successor node. The same index filters `G`, because currently pathable ground may become unsafe after pending work. Traversal on an already designated origin uses that designation's fixed profile and is not rejected by its own hourglass. Building footprints remain hard obstacles and are never traversable graph nodes; sharing the landslide index does not change that distinction. This generalizes the existing *Ramp safety margin* roadmap item into the graph itself as a hard inadmissibility rule, rather than a soft cost penalty.
...
* Precompute G nodes from the vanilla pathing flood before running the graph search. `E` is any G node in the tower-reachable flood.
* Start with Dijkstra (`heuristic = 0`) for the MVP. It is easier to validate because priority is exactly accumulated path cost.
* Keep A* available behind a public setting once the graph is validated. The first A* heuristic should be Manhattan tile distance from the current node to the nearest candidate end `E`. This ignores terrain work, so it is weak but admissible because every edge pays at least literal tile distance.
...
* A later stronger heuristic may add a lower bound on unavoidable height-gap work, but only if it remains admissible under zero-work reuse of existing designations and the chosen `workDistanceScale`.
